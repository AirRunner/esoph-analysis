...
# Graphically
library(corrplot)
esoph.prop.corr.plot <- cleanTable(esoph.prop.corr, NULL, 0.0)
corrplot(as.matrix(esoph.prop.corr.plot), type="full",
...
ggplot(esoph.prop, aes(x=y.alc, y=X)) + geom_point(size=1) +
  geom_smooth(method="lm") + stat_regline_equation(label.x=0.2, label.y=0.35) +
...
ggplot(esoph.prop, aes(x=y.tob, y=X)) + geom_point(size=1) +
  geom_smooth(method="lm") + stat_regline_equation(label.x=0.2, label.y=0.5) +
...
$L_full$ designates the maximized likelihood for the actual model, and $L_{null}$ designates the maximized likelihood for the null model, i.e. without predictors. These values are computed from the deviances given by the R model.
...
ggplot(data=prediction, aes(x=index, y=prob)) +
	geom_point(aes(color=cases), shape=4, stroke=2) +
	xlab("Index of predictions") + ylab("Predicted probability")
```
...
The model seems to predicts correct answers at 80%, which is really great. However, this test is not precise enough because we only test two cases: below and above or equal to $0.5$.
...
With this test, the model gives 60% correct predictions, which is good. Note that this value is similar to the $R^2$, but it should be a coincidence.
